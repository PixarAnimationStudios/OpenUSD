...
class LoFiTextureHandleRegistry::_TextureToHandlesMap
{
public:
    using TextureSharedPtr = LoFiTextureObjectSharedPtr;

    using HandlePtr = LoFiTextureHandlePtr;
    ...
    bool
    GarbageCollect(const std::set<LoFiTextureObjectSharedPtr> &textures) {
        bool result = false;

        for (LoFiTextureObjectSharedPtr const &texture : textures) {
            const typename Map::const_iterator it = _map.find(texture);
            if (it == _map.end()) {
                result = true;
            } else {
...
LoFiTextureHandleRegistry::LoFiTextureHandleRegistry(
    LoFiResourceRegistry * registry)
    : _textureTypeToMemoryRequestChanged(false)
    , _samplerObjectRegistry(
        std::make_unique<LoFiSamplerObjectRegistry>(registry))
    , _textureObjectRegistry(
        std::make_unique<LoFiTextureObjectRegistry>(registry))
    , _textureToHandlesMap(std::make_unique<_TextureToHandlesMap>())
{
}

LoFiTextureHandleRegistry::~LoFiTextureHandleRegistry() = default;

LoFiTextureHandleSharedPtr
LoFiTextureHandleRegistry::AllocateTextureHandle(
        LoFiTextureIdentifier const &textureId,
        ...
        LoFiShaderCodePtr const &shaderCode)
{
    ...
    LoFiTextureHandleSharedPtr const result =
        std::make_shared<LoFiTextureHandle>(
            textureObject,
            samplerParams,
            memoryRequest,
...
void
LoFiTextureHandleRegistry::MarkSamplerGarbageCollectionNeeded()
{
    _samplerObjectRegistry->MarkGarbageCollectionNeeded();
}

void
LoFiTextureHandleRegistry::MarkDirty(
...
void
LoFiTextureHandleRegistry::MarkDirty(
    LoFiShaderCodePtr const &shader)
{
...
void
LoFiTextureHandleRegistry::_ComputeMemoryRequest(
    LoFiTextureObjectSharedPtr const &texture)
{
    ...
    // handles.
    for (LoFiTextureHandlePtr const & handlePtr: *handles) {
        if(LoFiTextureHandleSharedPtr const handle = handlePtr.lock()) {
            maxRequest = std::max(
...
void
LoFiTextureHandleRegistry::_ComputeMemoryRequests(
    const std::set<LoFiTextureObjectSharedPtr> &textures)
{
    TRACE_FUNCTION();
...
void
LoFiTextureHandleRegistry::_ComputeAllMemoryRequests()
{
    TRACE_FUNCTION();
...
bool
LoFiTextureHandleRegistry::_GarbageCollectHandlesAndComputeTargetMemory()
{
    TRACE_FUNCTION();
    ...
    // the LoFiTextureObjectRegistry apply (see _Uniquify in
    // textureObjectRegistry.cpp).
    std::set<LoFiTextureObjectSharedPtr> dirtyTextures;
...
void
LoFiTextureHandleRegistry::_GarbageCollectAndComputeTargetMemory()
{
    TRACE_FUNCTION();
...
std::set<LoFiShaderCodeSharedPtr>
LoFiTextureHandleRegistry::_Commit()
{
    TRACE_FUNCTION();
    ...
    const std::set<LoFiTextureObjectSharedPtr>
    ...
    std::set<LoFiTextureHandleSharedPtr> dirtyHandles;
    _Uniquify(_dirtyHandles, &dirtyHandles);

    // ... texture handles associated to textures affected by the
    // (re)-commit.
    for (LoFiTextureObjectSharedPtr const & texture : committedTextures) {
    ...
    std::set<LoFiShaderCodeSharedPtr> result;

    {
        TRACE_FUNCTION_SCOPE(
            "Determining shader code instances that require updating.");

        for (LoFiTextureHandleSharedPtr const & handle : dirtyHandles) {

            // ... while allocating samplers for the textures.
            handle->ReallocateSamplerIfNecessary();

            if (LoFiShaderCodeSharedPtr const shader =
                        handle->GetShaderCode().lock()) {
                result.insert(shader);
            }
        }

        for (LoFiShaderCodePtr const & dirtyShader : _dirtyShaders) {
            if (LoFiShaderCodeSharedPtr const shader =
                        dirtyShader.lock()) {
                result.insert(shader);
            }
...
std::set<LoFiShaderCodeSharedPtr>
LoFiTextureHandleRegistry::Commit()
{
    ...
    const std::set<LoFiShaderCodeSharedPtr> result = _Commit();
...
void
LoFiTextureHandleRegistry::SetMemoryRequestForTextureType(
