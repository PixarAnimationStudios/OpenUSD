...
int32_t LoFiDualEdge::GetTriangle(short i) const 
{
...
int32_t LoFiDualEdge::GetPoint(short i) const 
{
  if(i == 0)return _halfEdge->vertex;
  else return _halfEdge->next->vertex;
}

LoFiDualEdge::LoFiDualEdge(const LoFiHalfEdge* halfEdge, bool facing,
...
void LoFiOctree::Split() 
{
  ...
  if (esz <= LOFI_OCTREE_MAX_EDGE_NUMBER || 
      (esz <= 2*LOFI_OCTREE_MAX_EDGE_NUMBER && _depth > 3) ||
      (esz <= 3*LOFI_OCTREE_MAX_EDGE_NUMBER && _depth > 4) ||
      _depth > 6 ) {
    _isLeaf = true;
    return;
  }
  ...
  for (int i = 0; i < 2; i++)
    for (int j = 0; j < 2; j++)
      for (int k = 0; k < 2; k++) 
      {
        int m = 4*i + 2*j + k;
        _children[m] = 
          new LoFiOctree( GfVec3f(xx[i], yy[j], zz[k]),
...
{
  const LoFiAdjacency* adjacency = _mesh->GetAdjacency();
  const GfVec3f*  triangleNormals = _mesh->GetTriangleNormalsPtr();
  const GfVec3f* positions = _mesh->GetPositionsPtr();
  size_t numPoints =  _mesh->GetNumPoints();
  size_t numTriangles =  _mesh->GetNumTriangles();

  const LoFiHalfEdge* twinEdge = halfEdge->twin;
  // check boundary
  if(! twinEdge)
  {
  ...
  // facing of the edge
  float ff = triangleNormals[halfEdge->triangle] * 
