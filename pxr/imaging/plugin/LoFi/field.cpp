...
// Unlicensed
//
#include "pxr/imaging/plugin/LoFi/field.h"
...
LoFiField::LoFiField(SdfPath const& id, TfToken const & fieldType) 
  : HdField(id)
...
LoFiField::~LoFiField() = default;

void
LoFiField::Sync(HdSceneDelegate *sceneDelegate,
                ...
                HdDirtyBits     *dirtyBits)
{
    ...
    if (*dirtyBits & DirtyParams) {
        ...
        // Resolve asset path
        //
        // Assuming that correct resolve context is set when LoFiField::Sync is
        // called.
        ...
        if (_fieldType == _tokens->openvdbAsset) {
            _textureId = LoFiTextureIdentifier(
                resolvedFilePath,
                std::make_unique<LoFiOpenVDBAssetSubtextureIdentifier>(
            ...
            _textureId = LoFiTextureIdentifier(
                resolvedFilePath,
                std::make_unique<LoFiField3DAssetSubtextureIdentifier>(
...
HdDirtyBits 
LoFiField::GetInitialDirtyBitsMask() const 
{
    return DirtyBits::AllDirty;
}

const TfTokenVector &
LoFiField::GetSupportedBprimTypes()
{
...
bool
LoFiField::IsSupportedBprimType(const TfToken &bprimType)
{
    return 
