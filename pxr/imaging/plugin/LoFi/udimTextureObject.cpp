...
bool LoFiIsSupportedUdimTexture(std::string const& imageFilePath)
{
...
LoFiUdimTextureObject::LoFiUdimTextureObject(
...
void
LoFiUdimTextureObject::_DestroyTextures()
{
    if (Hgi * hgi = _GetHgi()) {
        if (_texelTexture) {
...
LoFiUdimTextureObject::_Load()
{
    ...
    _hgiFormat = LoFiTextureUtils::GetHgiFormat(
        hioFormat,
        premultiplyAlpha);
    ...
    WorkParallelForN(tiles.size(), [&](size_t begin, size_t end) {
        for (size_t tileId = begin; tileId < end; ++tileId) {
            ...
            for (const HgiMipInfo &mipInfo : mipInfos) {
                ...
                LoFiTextureUtils::ReadAndConvertImage(
                    image,
                    /* flipped = */ true,
                    premultiplyAlpha,
                    mipInfo,
...
bool
LoFiUdimTextureObject::IsValid() const
{
...
HdTextureType
LoFiUdimTextureObject::GetTextureType() const
{
