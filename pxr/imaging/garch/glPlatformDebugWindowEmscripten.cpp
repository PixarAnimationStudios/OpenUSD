                                  ...
                                  int width, int height, int nSamples)
{
    /*EGLint attrib[] = {
        ...
        EGL_RENDERABLE_TYPE, GLX_WINDOW_BIT, //?
        GLX_DOUBLEBUFFER, True,
        EGL_RED_SIZE, 8,
        EGL_GREEN_SIZE, 8,
        EGL_BLUE_SIZE, 8,
        EGL_DEPTH_SIZE, 24,
        EGL_SAMPLE_BUFFERS, (nSamples > 1 ? 1 : 0),
    ...
    // X window
    int fbcount;
    EGLBoolean configResult = eglChooseConfig(_display, screen, attrib, &fbcount);
    if (!configResult) {
        TF_FATAL_ERROR("eglChooseConfig failed");
        exit(1);
    }
