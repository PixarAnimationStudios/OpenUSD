...
#define HDCYLINDER_1_SCHEMA_TOKENS \
    (cylinder_1) \
    (height) \
    (radiusTop) \
    (radiusBottom) \
...
TF_DECLARE_PUBLIC_TOKENS(HdCylinder_1SchemaTokens, HD_API,
    HDCYLINDER_1_SCHEMA_TOKENS);

//-----------------------------------------------------------------------------

class HdCylinder_1Schema : public HdSchema
{
public:
    HdCylinder_1Schema(HdContainerDataSourceHandle container)
    : HdSchema(container) {}
    ...
    /// Retrieves a container data source with the schema's default name token
    /// "cylinder_1" from the parent container and constructs a
    /// HdCylinderSchema instance.
    /// Because the requested container data source may not exist, the result
    /// should be checked with IsDefined() or a bool comparison before use.
    HD_API
    static HdCylinder_1Schema GetFromParent(
        const HdContainerDataSourceHandle &fromParentContainer);
