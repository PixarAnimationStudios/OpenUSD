...
HdStImplicitSurface::HdStImplicitSurface(SdfPath const &id,
                                         TfToken const &primType,
                                         HdBufferSpecVector customSpecs)
  : HdImplicitSurface(id)
  , _primType(primType)
  , _customSpecs(std::move(customSpecs))
  , _doubleSided(false)
...
HdStImplicitSurface::~HdStImplicitSurface() = default;

void
HdStImplicitSurface::UpdateRenderTag(HdSceneDelegate *delegate,
...
void
HdStImplicitSurface::Sync(HdSceneDelegate *delegate,
                          HdRenderParam   *renderParam,
...
void
HdStImplicitSurface::Finalize(HdRenderParam *renderParam)
{
    ...
    for (auto const& reprPair : _reprs) {
        ...
        _ImplicitSurfaceReprConfig::DescArray const &descs =
            _GetReprDesc(reprToken);
        HdReprSharedPtr repr = reprPair.second;
        int drawItemIndex = 0;
        for (const HdImplicitSurfaceReprDesc &desc : descs) {
            if (desc.geomStyle == HdImplicitSurfaceGeomStyleInvalid) {
                continue;
            }
...
void
HdStImplicitSurface::_UpdateRepr(HdSceneDelegate *sceneDelegate,
                                 HdRenderParam *renderParam,
                                 ...
                                 HdDirtyBits *dirtyBits)
{
    ...
    if (TfDebug::IsEnabled(HD_RPRIM_UPDATED)) {
        TfDebug::Helper().Msg(
            "HdStImplicitSurface::_UpdateRepr for %s : Repr = %s\n",
            GetId().GetText(), reprToken.GetText());
        HdChangeTracker::DumpDirtyBits(*dirtyBits);
    }

    _ImplicitSurfaceReprConfig::DescArray const &descs = _GetReprDesc(reprToken);
...
void
HdStImplicitSurface::_UpdateDrawItem(HdSceneDelegate *sceneDelegate,
                                     HdRenderParam *renderParam,
                                     ...
                                     HdDirtyBits *dirtyBits)
{
    ...
    if (HdStShouldPopulateConstantPrimvars(dirtyBits, id)) {
        ...
        HdStPopulateConstantPrimvars(this,
                                     ...
                                     drawItem,
                                     dirtyBits,
                                     nullptr, HdMeshGeomStyleInvalid,
                                     0, 0,
                                     _customSpecs,
...
void
HdStImplicitSurface::_UpdateDrawItemGeometricShader(
                                    ...
                                    HdImplicitSurfaceReprDesc const &desc)
{
    ...
    bool doubleSided = _doubleSided || desc.doubleSided;

    HdSt_ImplicitSurfaceShaderKey shaderKey{
        cullStyle, doubleSided, _vertexCount, _primType};
...
void
HdStImplicitSurface::_UpdateShadersForAllReprs(HdSceneDelegate *sceneDelegate,
                                               HdRenderParam *renderParam,
...
void
HdStImplicitSurface::_UpdateMaterialTagsForAllReprs(
                                           HdSceneDelegate *sceneDelegate,
                                           HdRenderParam *renderParam)
{
    TF_DEBUG(HD_RPRIM_UPDATED).Msg(
...
void
HdStImplicitSurface::_PopulateIndexBuffer(HdSceneDelegate *sceneDelegate,
                                     HdRenderParam *renderParam,
                                     HdStDrawItem *drawItem)
{
    HD_TRACE_FUNCTION();
...
HdDirtyBits
HdStImplicitSurface::GetInitialDirtyBitsMask() const
{
...
void
HdStImplicitSurface::_InitRepr(TfToken const &reprToken, HdDirtyBits *dirtyBits)
{
    ...
    *dirtyBits |= HdChangeTracker::NewRepr;
    ...
    for (const HdImplicitSurfaceReprDesc &desc : descs) {
        ...
        drawingCoord->SetInstancePrimvarBaseIndex(
            HdStImplicitSurface::InstancePrimvar);
    }
}

HdDirtyBits
HdStImplicitSurface::_PropagateDirtyBits(HdDirtyBits bits) const
{
    return bits;
}
