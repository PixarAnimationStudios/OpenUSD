...
TF_DEFINE_ENV_SETTING(HDST_ENABLE_NATIVE_CAPSULES, false,
    "Enable native rendering of capsule primitives in Storm instead of "
    "converting them to meshes via the implicit surface scene index.");
...
const TfTokenVector _SupportedRprimTypes()
{
    ...
    if (TfGetEnvSetting(HDST_ENABLE_NATIVE_CAPSULES)) {
...
bool
HdStRenderDelegate::IsEnabledNativeCapsuleRenderingSupport()
{
    return TfGetEnvSetting(HDST_ENABLE_NATIVE_CAPSULES);
}
