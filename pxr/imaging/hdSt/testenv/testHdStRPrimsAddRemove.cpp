...
void My_TestGLDrawing::AddRemoveBasisCurves(double &xPos, double& yPos)
{
    ...
    dmat.SetTranslate(GfVec3d(xPos, yPos, 0.0));
    delegate.AddCurves(SdfPath("/curve2"), HdTokens->cubic, HdTokens->bezier,
        GfMatrix4f(dmat), HdInterpolationVertex, HdInterpolationVertex);
    yPos += 3.0;

    dmat.SetTranslate(GfVec3d(xPos, yPos, 0.0));
    delegate.AddCurves(SdfPath("/curve3"), HdTokens->cubic, HdTokens->bSpline,
        GfMatrix4f(dmat), HdInterpolationVertex, HdInterpolationVertex);
    yPos += 3.0;

    dmat.SetTranslate(GfVec3d(xPos, yPos, 0.0));
    delegate.AddCurves(SdfPath("/curve4"), HdTokens->cubic, HdTokens->catmullRom,
        GfMatrix4f(dmat), HdInterpolationVertex, HdInterpolationVertex);
    yPos += 3.0;
    xPos += 3.0;

    dmat.SetTranslate(GfVec3d(xPos, yPos, 0.0));
    delegate.AddCurves(SdfPath("/curve5"), HdTokens->linear, TfToken(),
        GfMatrix4f(dmat), HdInterpolationVertex, HdInterpolationVertex);
    yPos += 3.0;
    ...
    delegate.Remove(SdfPath("/curve4"));
}
