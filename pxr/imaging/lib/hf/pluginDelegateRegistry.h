...
protected:
    ...
    ///
    HF_API HfPluginDelegateRegistry(const TfType &delegateBaseType);
    HF_API virtual ~HfPluginDelegateRegistry();

    ///
    /// Returns the delegate from the given delegateId.
    /// The reference count on the deletegate is automatically increased.
    ///
    HF_API HfPluginDelegateBase *GetDelegate(const TfToken &delegateId);
