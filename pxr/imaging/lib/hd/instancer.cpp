...
HdInstancer::GetInstancePrimVars(int level)
{
    ...
    if (HdChangeTracker::IsAnyPrimVarDirty(dirtyBits, instancerId)) {
        ...
        if (HdChangeTracker::IsAnyPrimVarDirty(dirtyBits, instancerId)) {
            ...
            primVarNames = _delegate->GetPrimVarInstanceNames(instancerId);
...
void
HdInstancer::_GetInstanceIndices(SdfPath const &prototypeId,
                                 std::vector<VtIntArray> *instanceIndicesArray)
{
    SdfPath const &instancerId = GetId();
    VtIntArray instanceIndices
        = _delegate->GetInstanceIndices(instancerId, prototypeId);
    ...
    if (!_parentId.IsEmpty()) {
        ...
        if (TF_VERIFY(parentInstancer)) {
            parentInstancer->_GetInstanceIndices(instancerId,
