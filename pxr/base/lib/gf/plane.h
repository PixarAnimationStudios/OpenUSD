...
//!
// \class GfPlane plane.h "pxr/base/gf/plane.h"
// \ingroup group_gf_BasicGeometry
// \brief Basic type: 3-dimensional plane
//
...
class GfPlane
{
  public:
    ...
    // normal and at \p distance units from the origin. The passed-in
    // normal is normalized to unit length first.
    ...
    //!
    // Sets this to the plane perpendicular to \p normal and at \p
    // distance units from the origin. The passed-in normal is
    ...
    //!
    // Component-wise equality test. The normals and distances must
    // match exactly for planes to be considered equal.
    ...
    //!
    // Component-wise inequality test. The normals and distances must
    // match exactly for planes to be considered equal.
    bool		operator !=(const GfPlane &p) const {
	return ! (*this == p);
    }
    ...
    //!
    // Returns the distance of point \p from the plane. This distance
    // will be positive if the point is on the side of the plane
    ...
    //!
    // Returns true if the given point is on the plane or within its 
    // positive half space.
