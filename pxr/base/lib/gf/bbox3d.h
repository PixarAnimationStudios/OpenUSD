...
//!
// \class GfBBox3d bbox3d.h "pxr/base/gf/bbox3d.h"
// \ingroup group_gf_BasicGeometry
// \brief Basic type: arbitrarily oriented 3D bounding box.
//
...
class GfBBox3d {
  ...
  public:
    ...
    //!  \brief The default constructor leaves the box empty, the
    // transformation matrix identity, and the \ref bbox3d_zeroAreaFlag
    // "zero-area primitives flag" \c false.
    GfBBox3d() {
        _matrix.SetIdentity();
    ...
    //!  \brief This constructor takes a box and sets the matrix to
    //identity.
    ...
    //!  \brief This constructor takes a box and a transformation
    //matrix.
    ...
    //!  \brief Sets the axis-aligned box and transformation matrix.
    void                Set(const GfRange3d &box, const GfMatrix4d &matrix) {
        _box = box;
        _SetMatrices(matrix);
    }

    //!  \brief Sets the transformation matrix only.  The axis-aligned
    //box is not modified.
    void                SetMatrix(const GfMatrix4d& matrix) {
        _SetMatrices(matrix);
    }

    //!  \brief Sets the range of the axis-aligned box only.  The transformation
    //matrix is not modified.
    void                SetRange(const GfRange3d& box) {
        _box = box;
    }

    //!  \brief Returns the range of the axis-aligned untransformed box.
    const GfRange3d &   GetRange() const {
        return _box;
    }

    //! \brief Returns the range of the axis-aligned untransformed box
    // This synonym of \c GetRange exists for 2x compatibility.
    ...
    //!  \brief Returns the inverse of the transformation matrix. This
    //will be the identity matrix if the transformation matrix is not
    //invertible.
    const GfMatrix4d &  GetInverseMatrix() const {
        return _inverse;
    }

    //!  \brief Sets the \ref bbox3d_zeroAreaFlag "zero-area primitives
    //flag" to the given value.
    void                SetHasZeroAreaPrimitives(bool hasThem) {
        _hasZeroAreaPrimitives = hasThem;
    }

    //!  \brief Returns the current state of the \ref
    //bbox3d_zeroAreaFlag "zero-area primitives flag" .
    ...
    //is assumed to be a global transformation to apply to the box.
    //Therefore, this just post-multiplies the box's matrix by \p
    //matrix.
    ...
    // results from applying the transformation matrix to the
    // axis-aligned box and aligning the result.
    // This synonym for \c ComputeAlignedRange exists for 2x compatibility.
    GfRange3d           ComputeAlignedBox() const {
        return ComputeAlignedRange();
    }

    //!  \brief Combines two bboxes, returning a new bbox that contains
    ...
    //equal. (To compare equality of the actual boxes, you can compute
    //both aligned boxes and test the results for equality.)
    bool                operator ==(const GfBBox3d &b) const {
        return (_box    == b._box &&
                _matrix == b._matrix);
    }

    //!  \brief Component-wise inequality test. The axis-aligned boxes and
    //transformation matrices match exactly for bboxes to be considered
    //equal. (To compare equality of the actual boxes, you can compute
    //both aligned boxes and test the results for equality.)
  ...
  private:
    ...
    //coordinate space to use to combine two boxes: \p b2 is transformed
    //into the space of \p b1 and the results are merged in that space.
