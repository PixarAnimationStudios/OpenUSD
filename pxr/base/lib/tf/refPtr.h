...
/*!
 * \class TfRefPtr RefPtr.h pxr/base/tf/refPtr.h
 * \ingroup group_tf_Memory
 * \brief Reference-counted smart pointer utility class
 *
 * The \c TfRefPtr class implements a reference counting on objects
 * that inherit from \c TfRefBase.  For more
 * information, see either the \ref refPtr_QuickStart "Quick Start"
...
public:
    ...
    /*!
     * \brief Initialize pointer to nullptr.
     *
     * The default constructor leaves the pointer initialized
     * to point to the NULL object.  Attempts to use the \c -> operator
     * will cause an abort until the pointer is given a value.
     */
    ...
    /*!
     * \brief Initializes \c *this to point at \p p's object.
     *
    ...
    /*!
     * \brief Initializes \c *this to point at \p gp's object.
     *
    ...
    /*!
     * \brief Transfer a raw pointer to a reference-counted pointer.
     *
     * The \c TfCreateRefPtr() function should only be used from within a
     * static \c New() function (or similarly, a \c Clone() function)
     * of a reference-counted class.  Reference-counted objects have
     * their reference count initially set to one to account for the
     * fact that a newly created object must always persist at least
     * until its \c New() function returns.  Therefore, the transfer
     * of the pointer returned by \c new into a reference pointer must
     * \e not increase the reference count.  The transfer of the raw
     * pointer returned by \c new into the object returned by \c New()
     * is a "transfer of ownership" and does not represent an
    ...
    /*!
     * \brief Initializes to point at \c *ptr.
     *
     * Increments \c *ptr's reference count.  Note that newly constructed
     * objects start with a reference count of one.  Therefore, you should \e NOT
     * use this constructor (either implicitly or explicitly) from
     * within a \c New() function.  Use \c TfCreateRefPtr() instead.
     */
    ...
    /*!
     * \brief Implicit conversion from \a TfNullPtr to TfRefPtr.
     */
    ...
    /*!
     * \brief Implicit conversion from \a nullptr to TfRefPtr.
     */
    ...
    /*!
     * \brief Assigns pointer to point at \c p's object, and increments
     * reference count.
     *
    ...
    /*!
     * \brief Decrements reference count of object being pointed to.
     *
...
public:
    ...
    /*!
     * \brief Initializes to point at \c p's object, and
    ...
    /*!
     * \brief Returns true if \c *this and \c p point to the same
     * object (or if they both point to NULL).
     *
    ...
    /*!
     * \brief Returns true if the address of the object pointed
     * to by \c *this compares less than the address of the object
     * pointed to by \p p.
     *
     * The comparison is legal only if a \c T* and a \c U* are comparable.
     */
    ...
    /*!
     * \brief Returns true if \c *this and \c p do not point to the same
     * object.
     *
     * The comparison is legal only if a \c T* and a \c U* are comparable.
     */
    ...
    /*!
     * Swap this pointer with \a other.  After this operation, this pointer will
     * point to what \a other formerly pointed to, and \a other will point to
     * what this pointer formerly pointed to.
     */
    ...
    /*!
     * Set this pointer to point to no object.  Equivalent to assignment with
     * TfNullPtr.
     */
    void Reset() {
        *this = TfNullPtr;
...
private:
    ...
    /*!
     * \brief Allows dynamic casting of a \c TfRefPtr.
     *
    ...
    /*!
     * \brief Allows static casting of a \c TfRefPtr.
     *
    ...
    /*!
     * \brief Allows const casting of a \c TfRefPtr.
     *
    ...
    /*!
     * \brief Call \c typeid on the object pointed to by a \c TfRefPtr.
     *
    ...
    // SGI Compiler forces us to make this a template declaration
    // of all TfTypeid's, to avoid linking bug (no instantiation is
    // generated).
