...
/*!
 * \class TfMallocTag
 * \brief Top-down memory tagging system.
 * \ingroup group_tf_MallocTag
 *
...
public:
    ...
    /*!
     * \struct CallTree
     * \brief Summary datastructure for malloc statistics.
     *
     * The \c CallTree structure is used to deliver a snapshot
     * of the current malloc usage.  It is accessible as publicly
     * modifiable data because it is simply a returned snapshot
     * of the current memory state.
     */
    struct CallTree {
        /*!
         * \struct PathNode
         * \brief Node in the call tree structure.
         * \ingroup group_tf_MallocTag
         *
         * See \ref page_tf_MallocTag for a detailed description.
         *
         ...
         * down the call tree).  Repeated call sites (in the case of co-recursive
         * function calls) can be skipped, e.g. pushing tags "A", "B", "C", "B", "C"
         * leads to only three path-nodes, representing the paths "A", "AB",
         * and "ABC".  Allocations done at the bottom (i.e. when tags
         * "A", "B", "C", "B", "C" are all active) are billed to the longest
         * path node in the sequence, which corresponds to the path "ABC").
         *
         * Path nodes track both the memory they incur directly (\c nBytesDirect)
         * but more importantly, the total memory allocated by themselves and
         * any of their children (\c nBytes).  The name of a node (\c siteName)
        ...
        /*!
         * \struct CallSite
         * \brief Record of the bytes allocated under each different tag.
         * \ingroup group_tf_MallocTag
         *
         * See \ref page_tf_MallocTag for a detailed description.
         *
         * Each construction of a \c TfAutoMallocTag object with a
         * different argument produces
         * a distinct \c CallSite record.  The total bytes outstanding for all
         * memory allocations made under a given call-site are recorded in
         * \c nBytes, while the name of the call site is available as \c name.
         */
        struct CallSite {
        ...
        //! \brief Specify which parts of the report to print.
        enum PrintSetting {
    ...
    /*
     * \struct CallStackInfo
     * \brief This struct is used to represent a call stack taken for an
     * allocation that was  billed under a specific malloc tag.
     */
    struct CallStackInfo
    {
...
public:
    ...
    /*
     * \class Auto
     * \brief Scoped (i.e. local) object for creating/destroying memory tags.
     *
     * Note: \c TfAutoMallocTag is a typedef to \c TfMallocTag::Auto; the
     * convention is to use \c TfAutoMallocTag to make it clear that
     * the local object exists only because its constructor and destructor
     ...
     * popping memory tags on the local call stack.  Most of the cost is simply
     * locking a mutex; typically, pushing or popping the call stack does not
     * actually cause any memory allocation unless this is the first time that
     * the given named tag has been encountered.
     *
     */
    ...
    public:
        /*!
         * \brief Push a memory tag onto the local-call stack with name \p name.
         *
         * If \c TfMallocTag::Initialize() has not been called, this constructor
         * does essentially no (measurable) work, assuming \p name is a string
         * literal or just a pointer to an existing string.
         *
         * Objects of this class should only be created as local variables; never
         * as member variables, global variables, or via \c new.  If you can't create
         * your object as a local variable, you can make manual calls to
         * \c TfMallocTag::Push() and \c TfMallocTag::Pop(), though you should
         ...
         *
         * If \c TfMallocTag::Initialize() has not been called, this constructor
         * does essentially no (measurable) work.  However, any work done in
         * constructing the \c std::string object \p name will be incurred
         * even if tagging is not active.  If this is an issue, you can query
         * \c TfMallocTag::IsInitialized() to avoid unneeded work when tagging
         * is inactive.  Note that the case when \p name is a string literal
         * does not apply here: instead, the constuctor that takes a \c const \c char*
        ...
        /*!
         * \brief Pop the tag from the stack before it is destructed.
         *
         * Normally you should not use this.  The normal destructor is
         * preferable because it insures proper release order.  If you
         * call \c Release(), make sure all tags are released in the
         * opposite order they were declared in.  It is better to use
         * sub-scopes to control the life span of tags, but if that
         * won't work, \c Release() is still preferable to \c
         * TfMallocTag::Push() and \c TfMallocTag::Pop() because it
         * isn't vulnerable to early returns or exceptions.
         */
        inline void Release() {
            if (_threadData) {
                _End();
        ...
        /*!
         * \brief Pop a memory tag from the local-call stack.
         *
         * If \c TfMallocTag::Initialize() was not called when this tag
         * was pushed onto the stack, popping the tag from the stack
         * does essentially no (measurable) work.
         */
        inline ~Auto() {
            Release();
        }
    ...
    public:
        /*!
         * \brief Push two memory tags onto the local-call stack.
         *
        ...
        /*!
         * \brief Pop two memory tags from the local-call stack.
         *
         * See TfMallocTag::Auto(const char* name).
         *
         */
    ...
    /*!
     * \brief Manually push a tag onto the stack.
     *
     * This call has the same effect as the constructor for
     * \c TfMallocTag::Auto (aka \c TfAutoMallocTag), however
     * a matching call to \c Pop() is required.
     *
     * Note that initializing the tagging system between matching
     * calls to \c Push() and \c Pop() is ill-advised, which is
     * yet another reason to prefer using \c TfAutoMallocTag whenever
     * possible.
     */
...
/*!
 * \hideinitializer
 * \brief Enable lib/tf memory management.
 * \ingroup group_tf_MallocTag
 *
 * See \ref page_tf_MallocTag for a detailed description.
 *
