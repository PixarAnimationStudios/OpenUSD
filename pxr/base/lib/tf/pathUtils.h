 ...
 * \brief Definitions of basic path utilities in tf.
 *
 * These are utilities that operate on paths (represented
 * by strings as something like: "/chars/Buzz/Torso".
 */

/*! \brief Returns the canonical path of the specified filename, eliminating any
 * symbolic links encountered in the path.
 *
 * This is a wrapper to realpath(3), which caters for situations where the real
 * realpath() would return a NULL string, such as the case where the path is
 * really just a program name.  The memory allocated by realpath is managed
 * internally.
 *
...
/*! \brief Normalizes the specified path, eliminating double slashes, etc.
 *
 * This canonicalizes paths, removing any double slashes, and
 ...
 * 
 * Examples: suppose the paths /, /usr, and /usr/anim exist, but no other paths
 * exist.
 *
 ...
 * filename.
 *
 * This makes the specified path absolute, by prepending
 * the current working directory.  If the path is already
 * absolute, it is returned unmodified.  This function differs
 * from TfRealPath in that the path may point to a symlink, or
 ...
 * necessary to glob a pattern, returning a std::vector of
 * results.  If no flags are specified, the GLOB_MARK and GLOB_NOCHECK
 * flags are set by default.  GLOB_MARK marks directories which match
 ...
 *
 * This form of Glob calls TfGlob.  For efficiency reasons, if
 * expanding more than one pattern, use the vector form.  As with
