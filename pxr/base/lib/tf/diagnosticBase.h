...
/*!
 * \class TfDiagnosticBase DiagnosticBase.h pxr/base/tf/diagnosticBase.h
 * \brief Represents the base class of an object representing a diagnostic
 *        message.
 ...
 * 
 * See \ref page_tf_TfError in the C++ API reference for a detailed
 * example.  For a description of how to post an error, warning or a
 * status message see \c TF_ERROR(), \c TF_WARN and \c TF_STATUS also in the
 * C++ API reference.
 *
 */
...
public:
    ...
    /*!
     * \brief Add to the commentary string describing
    ...
    /*!
     * \brief Return a (possibly NULL) const pointer to the info object
     * associated with this message.
     *
     * If this message was posted without supplying an \c info argument
     * to Post(), e.g.
     * \code
     ...
     * returned by GetInfo<T>().  If the type T doesn't match the held
     * type then GetInfo() returns NULL.
     */
    template <typename T>
    ...
    /*!
     * \brief Set the info object associated with this diagnostic message.
     * See \c GetInfo() for more details.
     */
    ...
    /*!
     * \brief Return true if the message was posted via \c PostQuietly().
     *
