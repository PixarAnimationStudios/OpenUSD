...
/*!
 * \file regTest.h
 * \brief Support for simple regression tests.
...
/*!
 * \class TfRegTest RegTest.h pxr/base/tf/regTest.h
 * \ingroup group_tf_Internal
 *
 * \c TfRegTest is a singleton class, which is used to register
 * functions with either type \c bool \c (*)(int, char*[]), or
 * functions returning type \c bool and taking no arguments.
 *
 * Here is how \c TfRegTest is used to create tests in separate files,
 * which are then compiled into a single standalone executable (and not
 * part of any library):
 *
 * // file: main.cpp
...
public:
    /*!
     * \brief Run a single regression test function, returning 0 if the
     * function succeeded and 1 otherwise.
     *
     ...
     * \endcode
     *
     * The first argument is the name of the test to be run.  If the registered
     * test function run takes no arguments, then no arguments other than
     * the test name should be supplied.  Otherwise, the \c Main() passes
     * \c argc-1 and \c argv+1 to the test function, and the test function
     * is responsible for argument checking.
     */
    ...
    /*!
     * \brief Type of a function with arguments.
     *
     * When \c Main(argc,argv) is requested to run a function of type
     * \c RegFuncWithArgs, it invokes the function with
     * arguments \c argc-1 and \c argv+1.
     */
