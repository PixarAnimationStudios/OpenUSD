...
/*!
 * \file vsnprintf.h
 * \brief Architecture dependent memory-safe sprintf capability
 * \ingroup group_arch_Strings
 */

/*!
 * \brief Return the number of characters (not including the null character)
 * necessary for a particular sprintf into a string.
 * \ingroup group_arch_Strings
 *
 * \c ArchVsnprintf guarantees the C99 behavior of vsnprintf on all
 * systems: it returns the number of bytes (not including the terminating
 * null character) needed to actually print the requested string.  If \c size
 ...
 * arguments. \c ArchVStringPrintf() does not call the \c va_end macro.
