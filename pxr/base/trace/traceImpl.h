...
#define TRACE_FUNCTION()                                                       \
  _TRACE_FUNCTION_INSTANCE(__LINE__, __ARCH_FUNCTION__,                        \
...
#define TRACE_FUNCTION_SCOPE(name)                                             \
  _TRACE_FUNCTION_SCOPE_INSTANCE(__LINE__, __ARCH_FUNCTION__,                  \
                                 __ARCH_PRETTY_FUNCTION__, name)
...
/// Records a counter \a delta using the \a name as the counter key. The delta
/// can be positive or negative. A positive delta will increment the total
/// counter value, whereas a negative delta will decrement it. The recorded
...
#define TRACE_FUNCTION_DYNAMIC(name)                                           \
  _TRACE_FUNCTION_DYNAMIC_INSTANCE(__LINE__, __ARCH_FUNCTION__,                \
                                   __ARCH_PRETTY_FUNCTION__, name)
...
#define _TRACE_FUNCTION_INSTANCE(instance, name, prettyName)                   \
  constexpr static PXR_NS::TraceStaticKeyData TF_PP_CAT(                       \
      TraceKeyData_, instance)(name, prettyName);                              \
...
#define _TRACE_FUNCTION_SCOPE_INSTANCE(instance, name, prettyName, scopeName)  \
  constexpr static PXR_NS::TraceStaticKeyData TF_PP_CAT(                       \
      TraceKeyData_, instance)(name, prettyName, scopeName);                   \
  PXR_NS::TraceScopeAuto TF_PP_CAT(TraceScopeAuto_, instance)(                 \
      TF_PP_CAT(TraceKeyData_, instance));
...
#define _TRACE_MARKER_INSTANCE(instance, name)                                 \
  constexpr static PXR_NS::TraceStaticKeyData TF_PP_CAT(TraceKeyData_,         \
                                                        instance)(name);       \
  TraceCollector::GetInstance().MarkerEventStatic(                             \
      TF_PP_CAT(TraceKeyData_, instance));

#define _TRACE_COUNTER_INSTANCE(instance, name, value, isDelta)                \
  constexpr static PXR_NS::TraceStaticKeyData TF_PP_CAT(TraceKeyData_,         \
                                                        instance)(name);       \
  static PXR_NS::TraceCounterHolder TF_PP_CAT(TraceCounterHolder_, instance)(  \
      TF_PP_CAT(TraceKeyData_, instance));                                     \
