...
static
std::string Arch_ReadProcStatusField(const char* field)
{
    ...
    for (std::string line; std::getline(procStatusFile, line);) {
        ...
        if (line.size() < fieldLen + 1 ) {
            continue;
        }

        if (strncmp(line.c_str(), field, fieldLen) == 0 && line[fieldLen] == ':') {
            // We found our "field:" line
            return line.substr(fieldLen + 2);
        }
    }
