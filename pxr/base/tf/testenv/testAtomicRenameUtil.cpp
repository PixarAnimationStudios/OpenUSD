...
namespace {
    ...
    // We're testing using TfSafeOutputFile::Replace, which first writes to a
    // temporary file, then moves it to the final file.  In our tests, our final
    // files end with a ".final" filename suffix.  This checks for the existence
    // of the temp files, by finding matches that DON'T have the ".final" suffix
    static size_t
...
//
// If a non-empty waitForFile is provided, then it will pause after the temp
// files are created, but before the file move is made, until the waitForFile
// exists.
//
...
{
    // We want to test Tf_AtomicRenameFileOver, but that's not exposed publicly,
    // so we test TfSafeOutputFile::Replace, which uses it
    ...
    if (!waitForFile.empty()) {
        ...
        auto maxTime = start +
            std::chrono::duration<double>(MAX_WAIT_FOR_FILE_SECONDS);
...
main(int argc, char *argv[])
{
    if (argc < 2) {
        ...
        std::cerr << "Usage: " << progName <<
            " FILE_BASE_NAME [WAIT_FOR_FILE]" << std::endl;
