...
# this timeout should be long enough to ensure that we either run out of
# retries, or finish successfully
TIMEOUT = 5.0
...
if platform.system() == "Windows":
    ...
    def launch_testAtomicRenameUtil(
        ...
        env = dict(os.environ)
        env["PXR_WINDOWS_FILE_ATOMIC_RENAME_RETRIES"] = str(
            NUM_RETRIES if do_retry else 0)
        env["PXR_WINDOWS_FILE_ATOMIC_RENAME_RETRY_WAIT_MS"] = str(
            int(retry_wait_seconds * 1000))
        args = [testAtomicRenameUtil_exe_path,
                os.path.join(folder, TEST_FILE_BASE)]
        if wait_for_file:
    ...
    def get_temp_files(folder: str) -> "List[str]":
        ...
        return [x for x in all_matches
                if x != os.path.join(folder, TEST_FILE_FINAL)]
    ...
    class TestAtomicRenameUtil(unittest.TestCase):
        ...
        def assert_new_content(self):
            with open(os.path.join(self.testingdir, TEST_FILE_FINAL),
                      "r", encoding="utf8") as reader:
        ...
        def _assert_testAtomicRenameUtil(
            ...
            expect_success: bool,
            wait_for_file: str = "",
            post_launch_callback:
                Optional[Callable[[subprocess.Popen], None]] = None,
        ) -> bool:
            """Run our testAtomicRenameUtil test executable, to verify correct
            operation of TfSafeOutputFile::Replace / TfAtomicRenameUtil

            Can optionally provide a wait_for_file to signal
            testAtomicRenameUtil to pause after creating temp files, but before
            doing the final move (and invoking TfAtomicRenameUtil).

            Can also provide a post_launch_callback, which will be executed
            after the testAtomicRenameUtil process is launched.  This can be
            used in conjunction with wait_for_file to do some actions at a point
            when we know the process is running, but before it has invoked
            TfAtomicRenameUtil.
            """
            expected_str = "succeed" if expect_success else "fail"
            # Print to sys.stderr, since that's what testAtomicRenameUtil will
            # print to if it fails
            print("#" * 80, file=sys.stderr, flush=True)
            print(f"Running {testAtomicRenameUtil_exe_path} "
                  f"- expected to {expected_str}", file=sys.stderr, flush=True)
            ...
            try:
                ...
                for temp_file in temp_files:
                    os.unlink(temp_file)
                print(f"  ...done running {testAtomicRenameUtil_exe_path}",
        ...
        def assertSuccess_testAtomicRenameUtil(
            ...
            self._assert_testAtomicRenameUtil(
                do_retry, True, wait_for_file=wait_for_file,
                post_launch_callback=post_launch_callback
            )
        ...
        def assertFailure_testAtomicRenameUtil(
            ...
            self._assert_testAtomicRenameUtil(
                do_retry, False, wait_for_file=wait_for_file,
                post_launch_callback=post_launch_callback
            )
        ...
        def test_fileLockedRetry(self):
            """Test retrying if another process has a lock on the file we wish
            to write over
            """
            # first, test that if no handles are open to test_file_final, it
            # works with or without retries
            ...
            with open(final_file, "w", encoding="utf8") as writer:
                ...
                # Now try running testAtomicRenameUtil without retries - it
                # should fail
                ...
                #     final file
                # - in the WRITING PROCESS:
                #   - we should finally succeed, because the lock has been
                #     released

                def wait_for_temp_then_create_then_release(
                        proc: subprocess.Popen):
                    # wait until a temp file shows up, so we know that the proc
                    # we launched is running
                    self.wait_for_temp_files(proc)
                    # create the wait_for_file
                    with open(sentry_file, "w", encoding="utf8") as _writer:
                        pass
                    # Do one more sleep, just to ensure we hang onto the lock
                    # for a bit
                    time.sleep(RETRY_WAIT_SEC)
                    # then release the lock
                    writer.close()

                # Now try launching testAtomicRenameUtil with retries, and pass
                # a callback which waits for temp files, then creates the
                # wait_for_file, then releases the lock
        ...
        def test_noFilePermsNoRetry(self):
            """Test that if we don't have permissions to move, we fail
            immediately without retry/waiting"""
            # set a long retry wait time, so we can be sure whether or not it
            # retried
            ...
            for do_retries in (False, True):
                sentry_path = os.path.join(
                    self.tempdir, f"sentry_file_do_retry_{do_retries}")

                # We need to make a directory that tests will be run in, that we
                # can lock; we need to leave the permissions on tempdir
                # writable, as that is where we will be writing our sentry file,
                # which needs to happen AFTER we lock the directory
                self.testingdir = os.path.join(
                    self.tempdir, f"do_retries_{do_retries}")
                os.mkdir(self.testingdir)

                # we want to test that what happens if there is a permissions
                # error when we try to "commit" the TfSafeOutputFile we do this
                # by locking down the directory perms on tempdir

                # However, if we do that before even launching
                # testAtomicRenameUtil, then it won't even be able to create the
                # temp files, and will never even get to the point where it
                # tries to commit

                # so we need to make a callback that locks down the directory
                # perms AFTER the proc is launched, and we can confirm temp
                # files were made
                ...
                try:
                    self.assertFailure_testAtomicRenameUtil(
                        ...
                        post_launch_callback=
                            wait_for_temp_then_lock_then_create,
                    )
                    timer_end = time.perf_counter()
                    # Check that it didn't retry, and failed quickly
                    self.assertLess(
                        timer_end - self.timer_start, self.retry_wait_seconds)
                finally:
