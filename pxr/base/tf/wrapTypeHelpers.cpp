...
{
    std::string moduleName = boost::python::extract<std::string>(classObj.attr("__module__"));
    std::string className = boost::python::extract<std::string>(classObj.attr("__name__"));
    std::string typeName = moduleName + "." + className;

    // Extract the bases, and declare them if they have not yet been declared.
    boost::python::object basesObj = classObj.attr("__bases__");
