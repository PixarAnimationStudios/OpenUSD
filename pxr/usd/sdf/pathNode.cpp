...
struct _PropToTokenTable
{
    ...
    FindOrCreate(Sdf_PathNode const *prop, Fn &&makeToken) {
        auto &propsToToks = _data->propsToToks;
        auto &mutex = _data->mutex;
        ...
        auto iter = propsToToks.find(prop);
        if (iter == propsToToks.end()) {
            ...
            // matter.
            iter = propsToToks.emplace(prop, std::move(tok)).first;
        }
        return iter->second;
    }
