...
public:
    ...
    // OFFSET 
    // --------------------------------------------------------------------- //
    /// The 2D offset from the position of the text primitive.
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `uniform float2 offset = (0, 0)` |
    /// | C++ Type | GfVec2f |
...
public:
    ...
    // MARGINS 
    // --------------------------------------------------------------------- //
    /// The margins at the four direction. The order is left margin, right margin,
...
public:
    ...
    // BLOCKALIGNMENT 
    // --------------------------------------------------------------------- //
    /// The text alignment in the block.
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `uniform token blockAlignment = "top"` |
    /// | C++ Type | TfToken |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Token |
    /// | \ref SdfVariability "Variability" | SdfVariabilityUniform |
    /// | \ref UsdTextTokens "Allowed Values" | top, center, bottom |
    USDTEXT_API
    UsdAttribute GetBlockAlignmentAttr() const;

    /// See GetBlockAlignmentAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDTEXT_API
    UsdAttribute CreateBlockAlignmentAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;

public:
    // --------------------------------------------------------------------- //
    // DIRECTION 
    // --------------------------------------------------------------------- //
    /// The direction of the text in the block. "leftToRight" and "rightToLeft" are valid only when 
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `uniform token direction = "defaultDir"` |
    /// | C++ Type | TfToken |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Token |
    /// | \ref SdfVariability "Variability" | SdfVariabilityUniform |
    /// | \ref UsdTextTokens "Allowed Values" | defaultDir, leftToRight, rightToLeft, topToBottom, bottomToTop |
    USDTEXT_API
...
public:
    ...
    // LINESFLOWDIRECTION 
    // --------------------------------------------------------------------- //
    /// The direction that the lines flow.
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `uniform token linesFlowDirection = "topToBottom"` |
    /// | C++ Type | TfToken |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Token |
    /// | \ref SdfVariability "Variability" | SdfVariabilityUniform |
    /// | \ref UsdTextTokens "Allowed Values" | topToBottom, bottomToTop, leftToRight, rightToLeft |
    USDTEXT_API
    UsdAttribute GetLinesFlowDirectionAttr() const;

    /// See GetLinesFlowDirectionAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDTEXT_API
    UsdAttribute CreateLinesFlowDirectionAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
