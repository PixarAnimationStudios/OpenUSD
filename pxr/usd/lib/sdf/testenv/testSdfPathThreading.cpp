...
_MakeRandomPath(SdfPath const &path = SdfPath::AbsoluteRootPath())
{
    ...
    // Extend a PrimPath.
    if (ret.IsPrimPath() and (random() & 1)) {
    ...
    if ((ret.IsPrimPath() or ret.IsPrimVariantSelectionPath())) {
        return (random() & 1) ? ret :
