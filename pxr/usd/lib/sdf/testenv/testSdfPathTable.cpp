...
static void DoUnitTest()
{
...

    {
        ...
        TF_AXIOM(table2.empty() and table3.empty());
    }
    ...
    // Find subtree range
    {
        ...
        TF_AXIOM(std::distance(range.first, range.second) == table.size());
                  ...
                  Driver *driver)
{
    ...
    // Insert all paths.
    size_t count = 0;
    BOOST_FOREACH(SdfPath const &path, paths) {
        driver->Insert(path);
        if (++count % 100000 == 0) {
...
struct HashAndSetDriver
{
    ...
    void EraseSubtree(SdfPath const &path) {
        ...
        while (*i == path or i->HasPrefix(path)) {
            hash.erase(*i);
            pathSet.erase(i++);
        }
    }
