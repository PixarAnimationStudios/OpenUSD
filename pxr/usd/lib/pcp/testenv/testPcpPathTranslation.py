...
def GetPcpCacheForLayer(rootLayerPath):
    rootLayerFile = FindDataFile(rootLayerPath)
    rootLayer = Sdf.Layer.FindOrOpen(rootLayerFile)
    Assert(rootLayer, "Cannot open layer %s" % (rootLayerPath))

    return Pcp.Cache(Pcp.LayerStackIdentifier(rootLayer))

############################################################

def TestPathTranslationWithVariants():
    """Tests path translation issues that were at the root of bug 77818"""
    print "TestPathTranslationWithVariants..."

    pcpCache = GetPcpCacheForLayer(
        "testPcpPathTranslation.testenv/bug77818/unit_main_cam.usda")
    ...
    AssertEqual(variantNode.path, Sdf.Path("/main_cam{lenses=Cooke_S4}Lens"))
    AssertEqual(Pcp.TranslatePathFromRootToNode(variantNode, "/main_cam/Lens"),
                Sdf.Path("/main_cam{lenses=Cooke_S4}Lens"))
    AssertEqual(Pcp.TranslatePathFromNodeToRoot(
                    variantNode, Sdf.Path("/main_cam{lenses=Cooke_S4}Lens")),
                Sdf.Path("/main_cam/Lens"))

TestPathTranslationWithVariants()

############################################################
# The following test cases are pulled from testCsdLayering8.py
############################################################

def TestLocalAbsoluteTargetPaths():
    print "TestLocalAbsoluteTargetPaths..."

    pcpCache = GetPcpCacheForLayer("testPcpPathTranslation.testenv/Root.usda")
    ...
    for (path, expectedTargetPath) in pathMap.items():
        ...
        AssertEqual(curTargetPaths, [Sdf.Path(expectedTargetPath)])

    AssertEqual(len(errors), 0)


TestLocalAbsoluteTargetPaths()

def TestLocalRelativeTargetPaths():
    ...
    for (path, expectedTargetPath) in pathMap.items():
        ...
        AssertEqual(curTargetPaths, 
                    [Sdf.Path(expectedTargetPath).MakeAbsolutePath(relPath.GetPrimPath())])

    AssertEqual(len(errors), 0)
    

TestLocalRelativeTargetPaths()

def TestReferenceAbsoluteTargetPaths():
...
TestReferenceAbsoluteTargetPaths()

def TestReferenceRelativeTargetPaths():
...
TestReferenceRelativeTargetPaths()

def TestReferenceErrorCases():
    ...
    for err in errors:
        print >> sys.stderr, err
        Assert(isinstance(err, Pcp.ErrorInvalidExternalTargetPath), 
               "Unexpected Error: %s" % err)

    AssertEqual(len(errors), len(paths))


TestReferenceErrorCases()
