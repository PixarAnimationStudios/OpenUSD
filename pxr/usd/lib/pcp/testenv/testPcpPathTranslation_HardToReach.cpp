...
{
    SdfLayerRefPtr rootLayer = SdfLayer::FindOrOpen(_FindDataFile(
        "testPcpPathTranslation_HardToReach.testenv/" + rootLayerPath));
...
{
    return node.GetArcType() == expectedArcType and
...
TestReverseTranslation_1()
{
    ...
    if (not pcpCache->GetPrimIndexInputs().cull) {
        ++nodeRange.first;
        // TF_AXIOM(_IsExpectedNode(
