...
TestPrim()
{
    ...
    unlink("foo.usdb");
    UsdStageRefPtr stage = UsdStage::CreateNew("foo.usdb");
...
TestIsDefined()
{
    ...
    TfToken attr1("attr1");
    TF_VERIFY(not p.GetAttribute(attr1).IsDefined());
    ...
    TF_VERIFY(not p.GetAttribute(attr1).IsAuthoredAt(strongLayer));
        ...
        double expectedMin = 0, double expectedMax = 0)
{
    ...
    if (not TF_VERIFY(attr.GetTimeSamples(&samples)))
        return;
    ...
    if (expectedNumSamples == 0) {
        ...
        TF_VERIFY(not hasTimeSamples);
        TF_VERIFY(numSamples==0);
    } else { 
        TF_VERIFY(not samples.empty());
...
TestValueMutation(std::string const & layerTag)
{
    ...
    // Empty initial state
    TF_VERIFY(not attr.Get(&value));

    // Ensure that attempting to set a value with incorrect type issues an
    // error.
    { ExpectedError err; TF_AXIOM(not attr.Set(1.234)); }
    ...
    // Has 
    TF_VERIFY(not attr.HasValue());
    TF_VERIFY(not attr.HasAuthoredValueOpinion());
    TF_VERIFY(not attr.HasMetadata(SdfFieldKeys->Default));
    TF_VERIFY(not attr.HasMetadata(SdfFieldKeys->TimeSamples));
    ...
    // Get with wrong type.
    { double d; TF_VERIFY(not attr.Get<double>(&d)); }
    { double d; TF_VERIFY(not attr.Get<double>(&d, 1.0)); }
    { double d; TF_VERIFY(not attr.Get<double>(&d, 2.0)); }
    ...
    TF_VERIFY(attr.ClearAtTime(1.0));
    TF_VERIFY(not attr.Get(&value, 1.0));
    // Has
    TF_VERIFY(not attr.HasValue());
    TF_VERIFY(not attr.HasAuthoredValueOpinion());
    TF_VERIFY(not attr.HasMetadata(SdfFieldKeys->Default));
    ...
    TF_VERIFY(not attr.Get(&value, 2.0));
    // Has
    TF_VERIFY(not attr.HasValue());
    TF_VERIFY(not attr.HasAuthoredValueOpinion());
    TF_VERIFY(not attr.HasMetadata(SdfFieldKeys->Default));
...
TestQueryTimeSample()
{
    SdfLayerRefPtr l = SdfLayer::CreateAnonymous("f.usdb");
...
TestVariability()
{
    ...
    
    {
        ...
        ExpectedError  err;
        TF_VERIFY(not foo.CreateAttribute(TfToken("configAttr"), 
                                          SdfValueTypeNames->Token,
