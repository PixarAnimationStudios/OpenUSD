...
template <>
struct TestCase<half>
{
    static void AddTestCase(const UsdPrim& prim)
    {
        UsdAttribute attr =
            prim.CreateAttribute(TfToken("testHalf"), SdfValueTypeNames->Half);
        TF_VERIFY(attr.Set(half(0.0f), UsdTimeCode(0.0)));
        TF_VERIFY(attr.Set(half(2.0f), UsdTimeCode(2.0)));
    }

    static void TestLinearInterpolation(const UsdPrim& prim)
    {
        UsdAttribute attr = prim.GetAttribute(TfToken("testHalf"));
        VerifyAttributeValue(attr, UsdTimeCode(0.0), half(0.0f));
        VerifyAttributeValue(attr, UsdTimeCode(1.0), half(1.0f));
        VerifyAttributeValue(attr, UsdTimeCode(2.0), half(2.0f));
    }
    ...
    static void TestHeldInterpolation(const UsdPrim& prim)
    {
        UsdAttribute attr = prim.GetAttribute(TfToken("testHalf"));
        VerifyAttributeValue(attr, UsdTimeCode(0.0), half(0.0f));
        VerifyAttributeValue(attr, UsdTimeCode(1.0), half(0.0f));
        VerifyAttributeValue(attr, UsdTimeCode(2.0), half(2.0f));
    }
};

template <>
struct TestCase<VtArray<half> >
{
    static void AddTestCase(const UsdPrim& prim)
    {
        UsdAttribute attr = 
            prim.CreateAttribute(TfToken("testHalfArray"), SdfValueTypeNames->HalfArray);
        TF_VERIFY(attr.Set(CreateVtArray(half(0.0f)), UsdTimeCode(0.0)));
        TF_VERIFY(attr.Set(CreateVtArray(half(2.0f)), UsdTimeCode(2.0)));
    }

    static void TestLinearInterpolation(const UsdPrim& prim)
    {
        UsdAttribute attr = prim.GetAttribute(TfToken("testHalfArray"));
        VerifyAttributeValue(attr, UsdTimeCode(0.0), CreateVtArray(half(0.0f)));
        VerifyAttributeValue(attr, UsdTimeCode(1.0), CreateVtArray(half(1.0f)));
        VerifyAttributeValue(attr, UsdTimeCode(2.0), CreateVtArray(half(2.0f)));
    }
    ...
    static void TestHeldInterpolation(const UsdPrim& prim)
    {
        UsdAttribute attr = prim.GetAttribute(TfToken("testHalfArray"));
        VerifyAttributeValue(attr, UsdTimeCode(0.0), CreateVtArray(half(0.0f)));
        VerifyAttributeValue(attr, UsdTimeCode(1.0), CreateVtArray(half(0.0f)));
        VerifyAttributeValue(attr, UsdTimeCode(2.0), CreateVtArray(half(2.0f)));
    }
};
...
TestInterpolationWithModelClips(const string &layerIdent)
{
    ...
    BOOST_FOREACH(UsdAttribute attr, mainTestPrim.GetAttributes()) {
        attr.Clear();
    }
