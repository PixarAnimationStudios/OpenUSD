...
//
#ifndef USDLIGHTFIELD_GENERATED_PARTICLEFIELD_H
#define USDLIGHTFIELD_GENERATED_PARTICLEFIELD_H

/// \file usdLightField/particleField.h
...
public:
    ...
    /// Construct a UsdLightFieldParticleField on UsdPrim \p prim .
    /// Equivalent to UsdLightFieldParticleField::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdLightFieldParticleField(const UsdPrim& prim=UsdPrim())
        : UsdGeomGprim(prim)
    {
    }

    /// Construct a UsdLightFieldParticleField on the prim held by \p schemaObj .
    /// Should be preferred over UsdLightFieldParticleField(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdLightFieldParticleField(const UsdSchemaBase& schemaObj)
    ...
    /// Destructor.
    USDLIGHTFIELD_API
    virtual ~UsdLightFieldParticleField();
    ...
    /// Return a UsdLightFieldParticleField holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdLightFieldParticleField(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDLIGHTFIELD_API
    static UsdLightFieldParticleField
