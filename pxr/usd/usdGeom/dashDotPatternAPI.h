...
public:
    ...
    /// previous symbol. If the current symbol is the first symbol, x must be zero. The y value saves the length of
    /// the current symbol. If it is zero, the current symbol is a dot. If it is larger than zero, the current 
    /// symbol is a dash. As a result, the total sum of all the x value and y value will be the length from the 
    /// start of the pattern to the end of the last symbol. This sum must be smaller than patternPeriod.
    /// 
