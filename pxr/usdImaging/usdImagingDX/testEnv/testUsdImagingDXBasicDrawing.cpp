...
class My_TestDXDrawing : public UsdImagingDX_UnitTestDXDrawing {
public:
    My_TestDXDrawing() {
    ...
    // UsdImagingDX_UnitTestDXDrawing overrides
...
void
My_TestDXDrawing::InitTest()
{
    TRACE_FUNCTION();
    
    std::cout << "My_TestDXDrawing::InitTest()\n";
...
My_TestDXDrawing::DrawTest(bool offscreen)
{
    ...
    std::cout << "My_TestDXDrawing::DrawTest()\n";
    ...
    if (GetCameraPath().empty()) {
        ...
        if (_AutoCamPosAndDir())
        {
           //
           // TODO: refactor away this copy/paste
           TfTokenVector purposes;
           purposes.push_back(UsdGeomTokens->default_);
           if (IsShowGuides()) {
              purposes.push_back(UsdGeomTokens->guide);
           ...
           //
           // I do not want to do anything about "stage z axis" for the moment
           // but I might revist this decision later

           GfVec3d vZ = GfVec3d(1, -1, 1);
           vZ.Normalize();
           GfVec3d vX = GfCross(GfVec3d(0, 0, 1), vZ);
           GfVec3d vY = GfCross(vZ, vX);

           GfVec3d ptCamPos = worldCenter + worldSize * vZ;

           GfMatrix4d cameraTrf(vX[0], vX[1], vX[2], 0,
                                vY[0], vY[1], vY[2], 0,
                                vZ[0], vZ[1], vZ[2], 0,
                                ptCamPos[0], ptCamPos[1], ptCamPos[2], 1);


           // TODO: figure out what I want to do about this later
           bool _bOrtho = true; 
           GfCamera camera(GfMatrix4d(1.0), _bOrtho ? GfCamera::Orthographic : GfCamera::Perspective);
           camera.SetTransform(cameraTrf);
           GfFrustum frustum = camera.GetFrustum();

           GfVec2d sceneCorner(0.6 * worldSize, 0.6 * worldSize);
           GfRange2d range(-sceneCorner, sceneCorner);
           frustum.SetWindow(range);
...
void
My_TestDXDrawing::ShutdownTest()
{
    std::cout << "My_TestDXDrawing::ShutdownTest()\n";
}

void
My_TestDXDrawing::MousePress(int button, int x, int y, int modKeys)
{
...
void
My_TestDXDrawing::MouseRelease(int button, int x, int y, int modKeys)
{
    _mouseButton[button] = 0;
}

void
My_TestDXDrawing::MouseMove(int x, int y, int modKeys)
{
