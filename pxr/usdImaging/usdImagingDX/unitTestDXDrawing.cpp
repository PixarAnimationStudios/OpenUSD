...
#include "unitTestDXDrawing.h"

#include "pxr/pxr.h"

#include "pxr/imaging/garch/glApi.h"

//#include "pxr/usdImaging/usdImagingDX/unitTestDXDrawing.h"
...
//
// TODO: need to refactor "presentation.h" to no longer need this here
...
class UsdImagingDX_UnitTestWindow : public GarchGLDebugWindow {
public:
    typedef UsdImagingDX_UnitTestWindow This;

public:
    UsdImagingDX_UnitTestWindow(UsdImagingDX_UnitTestDXDrawing * unitTest,
                                int w, int h);
    virtual ~UsdImagingDX_UnitTestWindow();
    ...
    static bool WriteToFile(const HgiTextureHandle& txHand, 
                            Hgi* pHgi,
                            std::string const & filename);
...
UsdImagingDX_UnitTestWindow::UsdImagingDX_UnitTestWindow(
    UsdImagingDX_UnitTestDXDrawing * unitTest, int w, int h)
...
UsdImagingDX_UnitTestWindow::OnInitializeGL()
{
    ...
    if (_unitTest->IsEnabledTestPresentOutput()) {
        ...
        desc.dimensions = GfVec3i(GetWidth(), GetHeight(), 1);
        desc.format = HgiFormat::HgiFormatUNorm8Vec4;
        desc.layerCount = 1;
...
/* virtual */
void
UsdImagingDX_UnitTestWindow::OnUninitializeGL()
{
...
/* virtual */
void
UsdImagingDX_UnitTestWindow::OnPaintGL()
{
    //
...
bool 
UsdImagingDX_UnitTestWindow::WriteToFile(const HgiTextureHandle& texture,
                                         Hgi* pHgi,
                                         std::string const & filename)
{
    size_t bufferSize = 0;
    ...
    TextureBuffer buffer = HdStTextureUtils::HgiTextureReadback<uint16_t>(pHgi, texture, &bufferSize);
...
bool
UsdImagingDX_UnitTestWindow::WriteToFile(std::string const & attachment,
                                         std::string const & filename)
{
...
/* virtual */
void
UsdImagingDX_UnitTestWindow::OnKeyRelease(int key)
{
    switch (key) {
    case 'q':
        ExitApp();
...
/* virtual */
void
UsdImagingDX_UnitTestWindow::OnMousePress(int button,
...
/* virtual */
void
UsdImagingDX_UnitTestWindow::OnMouseRelease(int button,
                                            int x, int y, int modKeys)
{
...
/* virtual */
void
UsdImagingDX_UnitTestWindow::OnMouseMove(int x, int y, int modKeys)
{
...
UsdImagingDX_UnitTestDXDrawing::UsdImagingDX_UnitTestDXDrawing()
    : _widget(NULL)
    , _testLighting(false)
    , _sceneLights(false)
    , _cameraLight(false)
...
int
UsdImagingDX_UnitTestDXDrawing::GetWidth() const
{
    return _widget->GetWidth();
}

int
UsdImagingDX_UnitTestDXDrawing::GetHeight() const
{
    return _widget->GetHeight();
}

bool
UsdImagingDX_UnitTestDXDrawing::WriteToFile(
    ...
    std::string const & filename)
{
    if (IsEnabledTestPresentOutput()) {
        ...
        return UsdImagingDX_UnitTestDXDrawing::WriteAovToFile(
                engine, aovName, filename);
    }
}

bool
UsdImagingDX_UnitTestDXDrawing::WriteAovToFile(
    UsdImagingGLEngine *engine,
    TfToken const &aovName,
    std::string const & filename)
{
    Hgi *pHgi = engine->GetHgi();
...
void
UsdImagingDX_UnitTestDXDrawing::_Parse(int argc, char *argv[], _Args* args)
{
    for (int i = 1; i != argc; ++i) {
        ...
        else if (strcmp(argv[i], "-autoCamPosAndDir") == 0) {
           _autoCamPosAndDir = true;
        }
...
/* virtual */
void
UsdImagingDX_UnitTestDXDrawing::MousePress(int button, int x, int y,
                                           int modKeys)
{
}
/* virtual */
void
UsdImagingDX_UnitTestDXDrawing::MouseRelease(int button, int x, int y,
                                             int modKeys)
{
}
/* virtual */
void
UsdImagingDX_UnitTestDXDrawing::MouseMove(int x, int y, int modKeys)
{
}
/* virtual */
void
UsdImagingDX_UnitTestDXDrawing::KeyRelease(int key)
{
}

void
UsdImagingDX_UnitTestDXDrawing::RunTest(int argc, char *argv[])
{
    _Args args;
    ...
    _widget = new UsdImagingDX_UnitTestWindow(
