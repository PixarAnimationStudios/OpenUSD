...
{
    TfType::Define<UsdImagingTextRenderer>();
}
...
UsdImagingTextRenderer::~UsdImagingTextRenderer() = default;

/// Initialize the text plugin using a text setting.
UsdImagingTextRendererSharedPtr UsdImagingTextRenderer::GetTextRenderer(const std::string& renderer)
{
    // Get and initialize a plugin from the registry.
    UsdImagingTextRendererRegistry& registry = UsdImagingTextRendererRegistry::GetInstance();
    return registry._GetTextRenderer(renderer);
}
