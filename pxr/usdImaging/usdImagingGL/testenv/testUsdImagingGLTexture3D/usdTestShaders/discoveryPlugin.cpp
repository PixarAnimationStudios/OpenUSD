...
_GetShaderResourcePath(char const * resourceName="")
{
    static PlugPluginPtr plugin = 
		PlugRegistry::GetInstance().GetPluginWithName("usdTestShaders"); 
...
const NdrStringVec& 
TestShadersDiscoveryPlugin::GetSearchURIs() const
{
...
TestShadersDiscoveryPlugin::DiscoverNodes(const Context &context)
{
...
NDR_REGISTER_DISCOVERY_PLUGIN(TestShadersDiscoveryPlugin);
