...
int main(int argc, char *argv[])
{
    ...
    std::vector<SdfPath> deactivatePaths;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--stage") {
            ...
            deactivatePaths.push_back(SdfPath(argv[++i]));
        } else if (arg == "--write") {
    ...
    light.SetPosition(GfVec4f(translate[0], translate[2], translate[1], 0));
    ...
    engine->SetLightingState(_lightingContext);

    // Deactivate texture nodes
    for (const SdfPath& path : deactivatePaths) {
        stage->GetPrimAtPath(path).SetActive(false);
    }

    engine->Render(stage->GetPseudoRoot(), params);

    // Reactivate texture nodes again
    for (const SdfPath& path : deactivatePaths) {
    ...
    UsdImagingGL_UnitTestGLDrawing::WriteAovToFile(
        engine.get(), HdAovTokens->color, imageFilePath + ".png");
