    ...
    with CurrentWorkingDirectory(DownloadURL(BOOST_URL, context, force, 
        ...
        if MacOS():
            if apple_utils.GetMacTargetArch(context) == apple_utils.TARGET_X86:
                ...
                macOSArch="-arch {0} -arch {1}".format(apple_utils.GetMacArmArch(), apple_utils.TARGET_X86)
    ...
    with CurrentWorkingDirectory(DownloadURL(TBB_URL, context, force)):
        ...
        if context.targetNative:
            ...
            if context.targetX86:
                primaryArch = apple_utils.TARGET_X86
        ...
        if context.targetUniversal:
            ...
            armFiles = glob.glob(os.getcwd() + "/build/*{0}*_release/libtbb*.*".format(apple_utils.GetMacArmArch()))
            ...
            armFiles = glob.glob(os.getcwd() + "/build/*{0}*_debug/libtbb*.*".format(apple_utils.GetMacArmArch()))
...
group.add_argument("--build-target", default=apple_utils.TARGET_NATIVE,
                   choices=[apple_utils.TARGET_NATIVE, apple_utils.TARGET_X86, apple_utils.TARGET_ARM64, apple_utils.TARGET_UNIVERSAL],
...
class InstallContext:
    def __init__(self, args):
        ...
        if apple_utils.SupportsMacOSUniversalBinaries():
            self.targetUniversal = (args.build_target == apple_utils.TARGET_UNIVERSAL)
        else:
...
summaryMsg = summaryMsg.format(
    ...
    buildTarget=(apple_utils.TARGET_NATIVE if context.targetNative
                  ...
                  else apple_utils.TARGET_UNIVERSAL if context.targetUniversal
                  else ""),
