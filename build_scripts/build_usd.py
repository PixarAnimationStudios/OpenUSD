...
def GetVisualStudioCompilerAndVersion():
    ...
    path possibly to be None, please don't depend on it"""
    if not Windows():
        return None
...
def FormatMultiProcs(numJobs, generator):
    ...
    if generator:
        ...
        if Windows():
            # Fallback setting for VisualStudio finding failure.
            # If no generator specified, clang or g++ was installed,
            # VisualStudio also installed but search fail, CMake
            # still use MSBuild to build.
            # After updating to sophisticated VisualStudio finding
            # approach, this branch should never be reached.
            tag = "/M:"
