...
if MacOS():
    ...
    # We should use different boost version depending on python version on Linux, 
    # and detecting python version need context to be initialized, so we set `BOOST_URL` later.
    BOOST_VERSION_FILE = "include/boost/version.hpp"
elif Windows():
...
def InstallBoost_Helper(context, force, buildArgs):
    ...
    if Linux():
        # We're using 1.70 to adhere to the VFX Reference Platform but are forced to use 1.78 to support 3.10.
        pythonInfo = GetPythonInfo(context)
        boostVersion = "1_78_0" if pythonInfo[3] == "3.10" else "1_70_0"
        BOOST_URL = "https://boostorg.jfrog.io/artifactory/main/release/1.78.0/source/boost_{}.tar.gz".format(boostVersion)
