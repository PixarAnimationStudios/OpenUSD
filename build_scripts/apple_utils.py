...
def GetFrameworkRoot():
    pathWithFramework = next(path for path in sys.path if ("Python.framework" in path or "Python3.framework" in path))
    splitPath = pathWithFramework.split('/')
    index = 0
    for p in range(0, len(splitPath)):
        if splitPath[index] == "Python.framework" or splitPath[index] == "Python3.framework":
            break
        index = index+1
