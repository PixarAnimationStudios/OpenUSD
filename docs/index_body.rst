.. include:: rolesAndUtils.rst

========
USD Home
========

.. grid:: 1
    :class-container: usd-title-image-container-width p-0 mb-4 sd-d-inline-block 

    .. grid-item-card:: 
       :class-body: sd-px-0 sd-py-0 usd-title-image-outer

       .. div:: +card-img-overlay +m-0 +p-2 +usd-title-image-inner

          |usdlogo| :usdtitletext:`Universal Scene Description`

       .. image:: https://openusd.org/images/piper-banner.jpg
          :class: +card-img

.. div:: usd-index-brief

   :fa:`caret-right` USD is a high-performance extensible software platform for
   collaboratively constructing animated 3D scenes, designed to meet the needs
   of large-scale film and visual effects production.

   :fa:`caret-right` USD enables robust interchange between digital content
   creation tools with its expanding set of schemas, covering domains like
   geometry, shading, lighting, and physics.

   :fa:`caret-right` USD's unique composition ability provides rich and varied
   ways to combine assets into larger assemblies, enables collaborative
   workflows so that many creators can work together with ease, and more.

.. grid:: 1 1 1 3
    :class-container: container-fluid p-0 mb-4
    :gutter: 1

    .. grid-item-card:: 
      :class-card: m-1
      :class-header: text-secondary font-weight-bold
      :shadow: md

      :fa:`power-off` Get Started
      ^^^^^^^^^^^
      - :doc:`intro`
      - :doc:`Tutorials <tut_usd_tutorials>`
      - :doc:`FAQ <usdfaq>`
      - :doc:`Toolset <toolset>`

    .. grid-item-card:: 
      :class-card: m-1
      :class-header: text-secondary font-weight-bold
      :shadow: md

      :fa:`download` Download
      ^^^^^^^^
      - `Get and Build USD <https://github.com/PixarAnimationStudios/OpenUSD/blob/release/README.md#getting-and-building-the-code>`_
      - Source: `Release <https://github.com/PixarAnimationStudios/OpenUSD/tree/release>`_ (`Changes <https://github.com/PixarAnimationStudios/OpenUSD/blob/release/CHANGELOG.md#change-log>`__)
      - Source: `Dev <https://github.com/PixarAnimationStudios/OpenUSD/tree/dev>`_ (`Changes <https://github.com/PixarAnimationStudios/OpenUSD/blob/dev/CHANGELOG.md#change-log>`__)
      - :ref:`Demo Assets <dl_downloads:Assets>`

    .. grid-item-card:: 
      :class-card: m-1
      :class-header: text-secondary font-weight-bold
      :shadow: md

      :fa:`book` Learn
      ^^^^^^^^^^^^^
      - `Learning Content on the USD Working Group Wiki <https://wiki.aswf.io/display/WGUSD/Learning+Content>`_
      - :doc:`Terms and Concepts <glossary>`
      - `API Documentation <api/index.html>`_
      - `OpenUSD Forum <https://forum.openusd.org>`_
      - :doc:`Specifications <spec>`
      - :doc:`Proposals <wp>`
   