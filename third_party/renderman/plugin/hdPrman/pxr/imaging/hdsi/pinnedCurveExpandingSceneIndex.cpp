...
// For versions without HdCopySampledDataSourceType cache the value at time zero
// so we don't recompute it.
class _CacheValueDataSource final : public HdSampledDataSource
{
public:
    HD_DECLARE_DATASOURCE(_CacheValueDataSource);

    /// input: the original data source
    _CacheValueDataSource(
        const HdSampledDataSourceHandle& input, 
        const VtValue& value)
        : _input(input)
        , _value(value)
    ...
    VtValue GetValue(Time shutterOffset) override
    {
        ...
        return _input->GetValue(shutterOffset);
    }
...
HD_DECLARE_DATASOURCE_HANDLES(_CacheValueDataSource);
...
struct _Visitor
{
    ...
    template <typename T>
    HdDataSourceBaseHandle operator()(const VtArray<T>& value)
    {
        return _TypedExpandedDataSource<T>::New(
            _CacheValueDataSource::New(_input, VtValue(value)), 
            _primvarName, _perCurveCounts, _numExtraEnds);
    }

    HdDataSourceBaseHandle operator()(const VtValue& value)
    {
        return _UntypedExpandedDataSource::New(
            _CacheValueDataSource::New(_input, value), 
            _primvarName, _perCurveCounts, _numExtraEnds);
    }
};
...
/* static */
HdPrmanPinnedCurveExpandingSceneIndexRefPtr
HdPrmanPinnedCurveExpandingSceneIndex::New(
    const HdSceneIndexBaseRefPtr &inputSceneIndex)
{
    return TfCreateRefPtr(
        new HdPrmanPinnedCurveExpandingSceneIndex(inputSceneIndex));
}


HdPrmanPinnedCurveExpandingSceneIndex::HdPrmanPinnedCurveExpandingSceneIndex(
    const HdSceneIndexBaseRefPtr &inputSceneIndex)
...
HdSceneIndexPrim
HdPrmanPinnedCurveExpandingSceneIndex::GetPrim(const SdfPath &primPath) const
{
...
SdfPathVector
HdPrmanPinnedCurveExpandingSceneIndex::GetChildPrimPaths(
...
void
HdPrmanPinnedCurveExpandingSceneIndex::_PrimsAdded(
...
void
HdPrmanPinnedCurveExpandingSceneIndex::_PrimsRemoved(
    const HdSceneIndexBase &sender,
...
void
HdPrmanPinnedCurveExpandingSceneIndex::_PrimsDirtied(
    const HdSceneIndexBase &sender,
