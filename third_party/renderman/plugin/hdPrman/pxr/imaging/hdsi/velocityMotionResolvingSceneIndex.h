...
//
#ifndef EXT_RMANPKG_PLUGIN_RENDERMAN_PLUGIN_HD_PRMAN_PXR_IMAGING_HDSI_VELOCITY_MOTION_RESOLVING_SCENE_INDEX_H
#define EXT_RMANPKG_PLUGIN_RENDERMAN_PLUGIN_HD_PRMAN_PXR_IMAGING_HDSI_VELOCITY_MOTION_RESOLVING_SCENE_INDEX_H

// XXX: Delete this file after hdPrman drops support for HD_API_VERSION versions  older than 106.

#include "pxr/imaging/hd/version.h"
#if HD_API_VERSION >= 106
#include <pxr/imaging/hdsi/velocityMotionResolvingSceneIndex.h> // IWYU pragma: export
#else
...
TF_DEBUG_CODES(
    HDPRMAN_VELOCITY_MOTION);

#define HDPRMAN_VELOCITY_MOTION_RESOLVING_SCENE_INDEX_TOKENS \
    (disable) \
    (enable) \
    (ignore) \
    (noAcceleration) \
...
TF_DECLARE_PUBLIC_TOKENS(HdPrmanVelocityMotionResolvingSceneIndexTokens,
    HDSI_API, HDPRMAN_VELOCITY_MOTION_RESOLVING_SCENE_INDEX_TOKENS);

TF_DECLARE_REF_PTRS(HdPrmanVelocityMotionResolvingSceneIndex);
...
/// appropriate.
///
class HdPrmanVelocityMotionResolvingSceneIndex
  : public HdSingleInputFilteringSceneIndexBase
{
public:
    HDSI_API
    static HdPrmanVelocityMotionResolvingSceneIndexRefPtr
    New(
...
protected:
    HdPrmanVelocityMotionResolvingSceneIndex(
        const HdSceneIndexBaseRefPtr& inputSceneIndex,
        const HdContainerDataSourceHandle& inputArgs = nullptr);
...
#endif // EXT_RMANPKG_PLUGIN_RENDERMAN_PLUGIN_HD_PRMAN_PXR_IMAGING_HDSI_VELOCITY_MOTION_RESOLVING_SCENE_INDEX_H