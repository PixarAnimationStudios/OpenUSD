    ...
    RtUString rileyTypeName)
{
    ...
    if (pval.IsHolding<float>()) {
        ...
        TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
            .Msg("      ri:intensity %f\n", intensity);
        filter_params->SetFloat(RtUString("intensity"), intensity);
    }
    ...
    if (filterType == _tokens->PxrIntMultLightFilter) {
        ...
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      ri:exposure %f\n", exposure);
            filter_params->SetFloat(RtUString("exposure"), exposure);
        }
    }
    ...
    if (filterType != _tokens->PxrIntMultLightFilter) {
        ...
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      ri:density %f\n", density);
            filter_params->SetFloat(RtUString("density"), density);
        }
    }
    ...
    if (pval.IsHolding<bool>()) {
        ...
        TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
            .Msg("      ri:invert %d\n", invert);
        filter_params->SetInteger(RtUString("invert"), invert);
    }
    ...
    if (pval.IsHolding<float>()) {
        ...
        TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
            .Msg("      ri:diffuse %f\n", diffuse);
        filter_params->SetFloat(RtUString("diffuse"), diffuse);
    }
    ...
    if (pval.IsHolding<float>()) {
        ...
        TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
            .Msg("      ri:specular %f\n", specular);
        filter_params->SetFloat(RtUString("specular"), specular);
    }
    pval = sceneDelegate->GetLightParamValue(filterPath,
    ...
    if (pval.IsHolding<TfToken>()) {
        ...
        //filter_params->SetFloat(RtUString("combineMode"), combineMode);
        // XXX -- what to do with this
    }

    // XX -- not sure how to handle this param
    //filter_params->SetString(RtUString("linkingGroups"), RtUString(""));
    // XX -- not sure how to handle this param
    ...
    if (filterType == _tokens->PxrIntMultLightFilter) {
        ...
        if (pval.IsHolding<float>()) {
            ...
            filter_params->SetFloat(RtUString("saturation"), saturation);
        }
        ...
        if (pval.IsHolding<TfToken>()) {
            ...
            if (barnMode == _tokens->physical)
                ...
                bm = 1;
            if (bm >= 0)
                filter_params->SetInteger(RtUString("barnMode"), bm);
        }
        ...
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      width %f\n", width);
            filter_params->SetFloat(RtUString("width"), width);
        }
        ...
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      height %f\n", height);
            filter_params->SetFloat(RtUString("height"), height);
        }
        ...
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      radius %f\n", radius);
            filter_params->SetFloat(RtUString("radius"), radius);
        }
        if (barnMode == _tokens->analytic) {
            ...
            if (pval.IsHolding<bool>()) {
                ...
                filter_params->SetInteger(RtUString("directional"),
                                          directional);
            }
            pval = sceneDelegate->GetLightParamValue(filterPath,
            ...
            if (pval.IsHolding<float>()) {
                ...
                filter_params->SetFloat(RtUString("shearX"), shearX);
            }
            pval = sceneDelegate->GetLightParamValue(filterPath,
            ...
            if (pval.IsHolding<float>()) {
                ...
                filter_params->SetFloat(RtUString("shearY"), shearY);
            }
            pval = sceneDelegate->GetLightParamValue(filterPath,
            ...
            if (pval.IsHolding<float>()) {
                ...
                TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                    .Msg("      analytic:apex %f\n", apex);
                filter_params->SetFloat(RtUString("apex"), apex);
            }
            pval = sceneDelegate->GetLightParamValue(filterPath,
            ...
            if (pval.IsHolding<bool>()) {
                ...
                TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                    .Msg("      analytic:useLightDirection %d\n",
                                            useLightDirection);
                filter_params->SetInteger(RtUString("useLightDirection"),
                                        useLightDirection);
            }
            pval = sceneDelegate->GetLightParamValue(filterPath,
            ...
            if (pval.IsHolding<float>()) {
                ...
                TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                    .Msg("      analytic:density:nearDistance %f\n",
                                        nearDistance);
                filter_params->SetFloat(RtUString("densityNear"), nearDistance);
            }
            pval = sceneDelegate->GetLightParamValue(filterPath,
            ...
            if (pval.IsHolding<float>()) {
                ...
                TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                    .Msg("      analytic:density:farDistance %f\n",
                                        farDistance);
                filter_params->SetFloat(RtUString("densityFar"), farDistance);
            }
            pval = sceneDelegate->GetLightParamValue(filterPath,
            ...
            if (pval.IsHolding<float>()) {
                ...
                filter_params->SetFloat(RtUString("densityNearVal"), nearValue);
            }
            pval = sceneDelegate->GetLightParamValue(filterPath,
            ...
            if (pval.IsHolding<float>()) {
                ...
                filter_params->SetFloat(RtUString("densityFarVal"), farValue);
            }
            pval = sceneDelegate->GetLightParamValue(filterPath,
            ...
            if (pval.IsHolding<float>()) {
                ...
                filter_params->SetFloat(RtUString("densityPow"), exponent);
            }
        }
        ...
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      edgeThickness %f\n", edgeThickness);
            filter_params->SetFloat(RtUString("edge"), edgeThickness);
        }
        pval = sceneDelegate->GetLightParamValue(filterPath,
        ...
        if (pval.IsHolding<TfToken>()) {
            ...
            if (preBarnEffect == _tokens->noEffect)
                ...
                preBarn = 1;
            if (preBarn >= 0)
                filter_params->SetInteger(RtUString("preBarn"), preBarn);
        }
        pval = sceneDelegate->GetLightParamValue(filterPath,
                                        _tokens->scaleWidth);
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      scaleWidth %f\n", width);
            filter_params->SetFloat(RtUString("scaleWidth"), width);
        }
        pval = sceneDelegate->GetLightParamValue(filterPath,
                                        _tokens->scaleHeight);
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      scaleHeight %f\n", height);
            filter_params->SetFloat(RtUString("scaleHeight"), height);
        }
        pval = sceneDelegate->GetLightParamValue(filterPath,
                                        _tokens->refineTop);
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      refine:top %f\n", top);
            filter_params->SetFloat(RtUString("top"), top);
        }
        pval = sceneDelegate->GetLightParamValue(filterPath,
        ...
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      refine:bottom %f\n", bottom);
            filter_params->SetFloat(RtUString("bottom"), bottom);
        }
        pval = sceneDelegate->GetLightParamValue(filterPath,
                                        _tokens->refineLeft);
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      refine:left %f\n", left);
            filter_params->SetFloat(RtUString("left"), left);
        }
        pval = sceneDelegate->GetLightParamValue(filterPath,
                                        _tokens->refineRight);
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      refine:right %f\n", right);
            filter_params->SetFloat(RtUString("right"), right);
        }
        if (edgeThickness > 0.0) {
            ...
            if (pval.IsHolding<float>()) {
                ...
                TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                    .Msg("      edgeScale:top %f\n", top);
                filter_params->SetFloat(RtUString("topEdge"), top);
            }
            pval = sceneDelegate->GetLightParamValue(filterPath,
            ...
            if (pval.IsHolding<float>()) {
                ...
                filter_params->SetFloat(RtUString("bottomEdge"), bottom);
            }
            pval = sceneDelegate->GetLightParamValue(filterPath,
            ...
            if (pval.IsHolding<float>()) {
                ...
                filter_params->SetFloat(RtUString("leftEdge"), left);
            }
            pval = sceneDelegate->GetLightParamValue(filterPath,
            ...
            if (pval.IsHolding<float>()) {
                ...
                filter_params->SetFloat(RtUString("rightEdge"), right);
            }
        }
        ...
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      depth %f\n", depth);
            filter_params->SetFloat(RtUString("depth"), depth);
        }
        pval = sceneDelegate->GetLightParamValue(filterPath, _tokens->radius);
        if (pval.IsHolding<float>()) {
            float radius = pval.UncheckedGet<float>();
        ...
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      scaleDepth %f\n", depth);
            filter_params->SetFloat(RtUString("scaleDepth"), depth);
        }
        pval = sceneDelegate->GetLightParamValue(filterPath,
                                        _tokens->refineTop);
        if (pval.IsHolding<float>()) {
        ...
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      refine:front %f\n", front);
            filter_params->SetFloat(RtUString("front"), front);
        }
        pval = sceneDelegate->GetLightParamValue(filterPath,
                                        _tokens->refineBack);
        if (pval.IsHolding<float>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      refine:back %f\n", back);
            filter_params->SetFloat(RtUString("back"), back);
        }
        if (edgeThickness > 0.0) {
            pval = sceneDelegate->GetLightParamValue(filterPath,
                                            _tokens->edgeScaleTop);
        ...
        if (pval.IsHolding<std::vector<float>>()) {
            ...
            filter_params->SetInteger(RtUString("falloff"), v.size());
        }
        pval = sceneDelegate->GetLightParamValue(filterPath,
        ...
        if (pval.IsHolding<TfToken>()) {
            ...
            filter_params->SetString(RtUString("falloff_Interpolation"),
        ...
        if (pval.IsHolding<std::vector<float>>()) {
            ...
            filter_params->SetFloatArray(RtUString("colorRamp_Knots"),
                                         &v[0], v.size());
            // XXX -- extra param to hold the size of the spline
            filter_params->SetInteger(RtUString("colorRamp"), v.size());
        }
        pval = sceneDelegate->GetLightParamValue(filterPath,
        ...
        if (pval.IsHolding<TfToken>()) {
            ...
            TF_DEBUG(HDPRMAN_LIGHT_FILTER_LINKING)
                .Msg("      colorRamp:spline:interpolation %s\n",
                                        interpolation.GetText());
            filter_params->SetString(RtUString("colorRamp_Interpolation"),
                                     RtUString(interpolation.GetText()));
        }
        ...
        mgr->DestroyRixParamList(filter_params);
        return false;
    }
    ...
    if (genCoordSys) {
        ...
        filter_params->SetString(RtUString("coordsys"),
                                 RtUString(csName));
        if (genParentShader)
            filter_params->SetString(RtUString("__lightFilterParentShader"),
                                     rileyTypeName);
    }
    return true;
}
