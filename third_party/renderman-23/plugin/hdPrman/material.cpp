    ...
    for (size_t i=0; i < mat.nodes.size(); ++i) {
        ...
        for (const auto param: mat.nodes[i].parameters) {
            ...
            if (propType == SdrPropertyTypes->Struct ||
                ...
                // RtParamList (specifically, RtDataType) doesn't have
    ...
    std::vector<riley::ShadingNode> *result)
{
    ...
    _MapHdNodesToRileyNodes(mat, shaders, result);
    return !result->empty();
}
...
{  
    ...
    if ((*dirtyBits & HdMaterial::DirtyResource) ||
        ...
        if (vtMat.IsHolding<HdMaterialNetworkMap>()) {
            ...
            // Surface/Volume
            if (_ConvertHdMaterialToRman(surfaceNet, &nodes) ||
                _ConvertHdMaterialToRman(volNet, &nodes)) {
                if (_materialId == riley::MaterialId::k_InvalidId) {
            ...
            // Displacement
            if (_ConvertHdMaterialToRman(dispNet, &nodes)) {
                if (_displacementId == riley::DisplacementId::k_InvalidId) {
