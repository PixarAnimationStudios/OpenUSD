    ...
    RtParamList& options)
{
    ...
    for (auto const& entry : renderSettings) {
        ...
        if (hasRiPrefix) {
            ...
            if (token == HdPrmanRenderSettingsTokens->dataWindowNDC) {
/*                // XXX It looks like husk is handling this?
                // Reorder dataWindowNDC to match prman crop window convention
                // USD convention: (xmin, ymin, xmax, ymax)
                // RenderMan convention: (xmin, xmax, ymin, ymax)
                float rmanCropWindow[4];
                auto const& usdCropWindow = val.UncheckedGet<GfVec4f>();
                rmanCropWindow[0] = usdCropWindow[0];   // xmin
                rmanCropWindow[1] = usdCropWindow[2];   // xmax
                rmanCropWindow[2] = usdCropWindow[1];   // ymin
                rmanCropWindow[3] = usdCropWindow[3];   // ymax
                options.SetFloatArray(RixStr.k_Ri_CropWindow, 
                                      &rmanCropWindow[0], 4);*/
