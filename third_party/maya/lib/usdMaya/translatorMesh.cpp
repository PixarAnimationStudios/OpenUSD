        ...
        PxrUsdMayaPrimReaderContext* context)
{
    ...
    if (faceVertexCounts.size() == 0 or faceVertexIndices.size() == 0) {
        MGlobal::displayError(
    ...
    TF_FOR_ALL(iter, primvars) {
        ...
        // if it has the Authored custom data and it's set to true. If not skip since
        // the primvar was generated by shader values and not an authored colorset/entity
        // displayColor and displayOpacity are required to provide the Authored custom data
        if (name == "displayColor" or name == "displayOpacity") {
            if (not PxrUsdMayaUtil::GetBoolCustomData(primvar.GetAttr(), TfToken("Authored"), false)) {
                continue;
            }
        }
        ...
        if (typeName == SdfValueTypeNames->Float2Array) {
            ...
            if (typeName == SdfValueTypeNames->FloatArray) {
                if (_AssignColorSetPrimvarToMesh(mesh, primvar, meshFn, polygonCounts, polygonConnects, MFnMesh::kAlpha)==false) {
                    MGlobal::displayWarning(TfStringPrintf("Unable to retrieve and assign data for Alpha ColorSet <%s> on mesh <%s>", 
                                name.GetText(), mesh.GetPrim().GetPath().GetText()).c_str());                    
                }
            } else if (typeName == SdfValueTypeNames->Color3fArray) {
                if (_AssignColorSetPrimvarToMesh(mesh, primvar, meshFn, polygonCounts, polygonConnects, MFnMesh::kRGB)==false) {
                    MGlobal::displayWarning(TfStringPrintf("Unable to retrieve and assign data for RGB ColorSet <%s> on mesh <%s>", 
                            name.GetText(), mesh.GetPrim().GetPath().GetText()).c_str());                    
                }
            } else if (typeName == SdfValueTypeNames->Float4Array) {
                if (_AssignColorSetPrimvarToMesh(mesh, primvar, meshFn, polygonCounts, polygonConnects, MFnMesh::kRGBA)==false) {
                        MGlobal::displayWarning(TfStringPrintf("Unable to retrieve and assign data for RGBA ColorSet <%s> on mesh <%s>", 
                                name.GetText(), mesh.GetPrim().GetPath().GetText()).c_str());                    
                }
            }
        }
    ...
    if (meshFn.getColorSetNames(colorSetNames)==MS::kSuccess) {
        for (unsigned int i=0; i < colorSetNames.length(); i++) {
            ...
            if (csRep==MFnMesh::kRGB or csRep==MFnMesh::kRGBA) {
