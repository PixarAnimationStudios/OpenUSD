...
static void _RenderCallback(HdParticleFieldRenderer* renderer, HdRenderThread* renderThread) {
    renderer->Clear();
    renderer->Render(renderThread);
}

HdParticleFieldRenderDelegate::HdParticleFieldRenderDelegate() : HdRenderDelegate() { _Setup(); }

HdParticleFieldRenderDelegate::HdParticleFieldRenderDelegate(const HdRenderSettingsMap& settingsMap)
...
void HdParticleFieldRenderDelegate::_Setup() {
    ...
    _renderParam = std::make_unique<HdParticleFieldRenderParam>(&_renderer, &_renderThread, &_sceneVersion);
    ...
    _resourceRegistry = std::shared_ptr<HdResourceRegistry>(new HdResourceRegistry());
}
...
TfTokenVector const& HdParticleFieldRenderDelegate::GetSupportedRprimTypes() const { return SUPPORTED_RPRIM_TYPES; }

TfTokenVector const& HdParticleFieldRenderDelegate::GetSupportedSprimTypes() const { return SUPPORTED_SPRIM_TYPES; }

TfTokenVector const& HdParticleFieldRenderDelegate::GetSupportedBprimTypes() const { return SUPPORTED_BPRIM_TYPES; }

HdRenderParam* HdParticleFieldRenderDelegate::GetRenderParam() const { return _renderParam.get(); }

HdRenderSettingDescriptorList HdParticleFieldRenderDelegate::GetRenderSettingDescriptors() const {
    return _settingDescriptors;
}

HdResourceRegistrySharedPtr HdParticleFieldRenderDelegate::GetResourceRegistry() const { return _resourceRegistry; }

HdRenderPassSharedPtr HdParticleFieldRenderDelegate::CreateRenderPass(HdRenderIndex* index,
                                                                      const HdRprimCollection& collection) {
    return HdRenderPassSharedPtr(
        new HdParticleFieldRenderPass(index, collection, &_renderer, &_renderThread, &_sceneVersion));
}
...
HdSprim* HdParticleFieldRenderDelegate::CreateSprim(const TfToken& typeId, const SdfPath& sprimId) {
...
HdSprim* HdParticleFieldRenderDelegate::CreateFallbackSprim(const TfToken& typeId) {
    if (typeId == HdPrimTypeTokens->camera) {
        return new HdCamera(SdfPath::EmptyPath());
    } else {
        TF_CODING_ERROR("Unknown Fallback Sprim type=%s id=%s", typeId.GetText(), typeId.GetText());
    }
    return nullptr;
}

void HdParticleFieldRenderDelegate::DestroySprim(HdSprim* sprim) { delete sprim; }

HdBprim* HdParticleFieldRenderDelegate::CreateBprim(const TfToken& typeId, const SdfPath& bprimId) {
...
HdBprim* HdParticleFieldRenderDelegate::CreateFallbackBprim(const TfToken& typeId) {
    if (typeId == HdPrimTypeTokens->renderBuffer) {
...
void HdParticleFieldRenderDelegate::DestroyBprim(HdBprim* bprim) { delete bprim; }

HdInstancer* HdParticleFieldRenderDelegate::CreateInstancer(HdSceneDelegate* delegate, const SdfPath& id) {
...
HdAovDescriptor HdParticleFieldRenderDelegate::GetDefaultAovDescriptor(const TfToken& aovName) const {
